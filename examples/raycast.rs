...
impl State for Raycast {
    fn new() -> Result<Raycast> {
        ...
        let regions = vec![Rectangle::new_sized(800, 600),
        ...
        if let &Event::MouseMoved(_) = event {
            ...
            for i in 0..self.targets.len() {
                ...
                let mut cast_ray = |direction: f32| {
                    ...
                    // Perform the actual raycast, returning the target and an iterator of
                    // collisions
                    ...
                    let cast =
                        self.regions.iter()
                            ...
                            .min_by(|a: &Vector, b: &Vector| {
                                        distance_to(a).partial_cmp(&distance_to(b))
                                                      .unwrap_or(Ordering::Equal)
                                    });
                    if let Some(pos) = cast {
                        self.vertices.push(Vertex { pos,
                                                    tex_pos: None,
                                                    col: Color::white(), });
                    }
                };
            ...
            self.vertices.sort_by(|a, b| {
                                      angle_to(a).partial_cmp(&angle_to(b))
                                                 .unwrap_or(Ordering::Equal)
                                  });
...
fn main() { run::<Raycast>(WindowBuilder::new("Raycast", 800, 600)).unwrap(); }
