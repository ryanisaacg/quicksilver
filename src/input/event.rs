...
#[cfg(not(target_arch = "wasm32"))]
impl EventProvider {
    pub(crate) fn new(events_loop: EventsLoop) -> EventProvider { EventProvider { events_loop } }
        ...
        self.events_loop.poll_events(|event| match event {
            WindowEvent { event, .. } => match event {
                ...
                glutin::WindowEvent::CursorMoved { position, .. } => {
                    let (x, y) = position;
                    let position = (Vector::new(x as f32, y as f32) - window.screen_offset())
                        / window.scale_factor;
                ...
                glutin::WindowEvent::MouseWheel { delta, .. } => {
                    let (x, y) = match delta {
                        glutin::MouseScrollDelta::LineDelta(x, y) => {
                            (x * LINES_TO_PIXELS, y * -LINES_TO_PIXELS)
                        }
