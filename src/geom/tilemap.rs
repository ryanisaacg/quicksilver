use geom::{Positioned, Rectangle, Shape, Vector};
use std::ops::Fn;
...
impl<T: Clone> Tile<T> {
    ...
    pub fn empty(value: Option<T>) -> Tile<T> { Tile { value, empty: true } }
}
...
impl<T: Clone> Tilemap<T> {
    ...
    pub fn width(&self) -> f32 { self.map_size.x }

    ///Get the height of the map
    pub fn height(&self) -> f32 { self.map_size.y }

    ///Get the size of the map
    pub fn size(&self) -> Vector { self.map_size }

    ///Get the region the map takes up
    pub fn region(&self) -> Rectangle { Rectangle::newv_sized(self.map_size) }

    ///Get the width of an individual tile
    pub fn tile_width(&self) -> f32 { self.tile_size.x }

    ///Get the height of an individual tile
    pub fn tile_height(&self) -> f32 { self.tile_size.y }

    ///Get the size of a tile
    pub fn tile_size(&self) -> Vector { self.tile_size }

    ///Check if a point is within the map bounds
    pub fn valid(&self, index: Vector) -> bool { self.region().contains(index) }
    ...
    pub fn shape_empty(&self, shape: Shape) -> bool {
        ...
        match shape {
            ...
            Shape::Rectangle(_) | Shape::Circle(_) => {
                ...
                let y_end =
                    (self.align_right(bounds.y + bounds.height) / self.tile_height()) as i32;
                for x in x_start..x_end {
                    for y in y_start..y_end {
                        let point = Vector::new(x, y).times(self.tile_size());
                        if !self.point_empty(point)
                           && shape.overlaps_rect(Rectangle::newv(point, self.tile_size()))
                        {
                            return false;
                        }
                    }
    ...
    pub fn align_left(&self, x: f32) -> f32 { (x / self.tile_width()).floor() * self.tile_width() }

    ///Align a given X value to the rightmost edge of a tile
    pub fn align_right(&self, x: f32) -> f32 { (x / self.tile_width()).ceil() * self.tile_width() }

    ///Align a given Y value to the topmost edge of a tile
    pub fn align_top(&self, y: f32) -> f32 { (y / self.tile_height()).floor() * self.tile_height() }
    ...
    ///Find the furthest a shape can move along a vector, and what its future
    /// speed should be
    pub fn move_until_contact(&self, bounds: Shape, speed: Vector) -> (Shape, Vector) {
        ...
        let returned_speed = Vector::new(if attempt.x == speed.x {
                                             attempt.x
                                         } else {
                                             0.0
                                         },
                                         if attempt.y == speed.y {
                                             attempt.y
                                         } else {
                                             0.0
                                         });
              ...
              F: Fn(&T) -> U
    {
        Tilemap { data: self.data
                            .iter()
                            .map(|tile| Tile { value: match tile.value {
...
mod tests {
    ...
    fn move_until_contact() {
        let map = setup();
        //Each test case has a starting rectangle, starting speed, expected top-left
        // and expected speed
        let test_cases =
            [(Rectangle::new(64, 64, 10, 10),
             ...
             Vector::new(12, 7),
             Vector::new(2, 2)),
             (Rectangle::new(5, 5, 10, 10), Vector::x() * -7.2, Vector::new(0, 5), Vector::zero()),
             (Rectangle::new(0, 0, 30, 30), Vector::x() * 2, Vector::new(2, 0), Vector::x() * 2),
             ...
             Vector::x() * 50,
             Vector::x() * -100),
             (Rectangle::new(0, 150, 30, 30), Vector::y() * -200, Vector::zero(), Vector::zero())];
