...
impl Font {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> FontLoader {
        FontLoader(FileLoader::load(path).map(parse as LoadFunction))
    }

    /// Creates font from bytes sequence.
    pub fn from_slice(data: &'static [u8]) -> Result<Self, QuicksilverError> {
        Ok(Font { data: FontCollection::from_bytes(data)?.into_font()?, })
    }
    ...
    ///
    /// This function does not take into account unicode normalization or
    /// vertical layout
    pub fn render(&self, text: &str, style: FontStyle) -> Image {
        let scale = Scale { x: style.size,
                            y: style.size, };
        //Avoid clipping
        let offset = point(0.0, self.data.v_metrics(scale).ascent);
        let glyphs = self.data
        ...
        Image::from_raw(pixels.as_slice(),
                        width as u32,
                        style.size as u32,
                        PixelFormat::RGBA)
    }
...
fn parse(data: Vec<u8>) -> Result<Font, QuicksilverError> { Font::from_bytes(data) }
...
impl FontStyle {
    ...
    pub fn new(size: f32, color: Color) -> FontStyle { FontStyle { size, color } }
}
