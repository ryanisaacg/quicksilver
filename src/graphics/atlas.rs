...
use graphics::{Image, ImageError, ImageLoader};
use std::{
    cmp::Ordering,
    collections::HashMap,
    error::Error,
...
#[derive(Clone, Debug)]
/// An image atlas that allows a single image file to represent multiple
/// individual images
///
/// It uses the libgdx / spine atlas format and groups them by name and index
/// if applicable
pub struct Atlas {
...
impl Atlas {
    ...
    pub fn get(&self, name: &str) -> Option<AtlasItem> { Some(self.data.get(name)?.clone()) }
}
...
fn create(data: (Vec<Image>, Vec<Region>)) -> Atlas {
    ...
    images.sort_by(|a: &(String, i32, Image), b: &(String, i32, Image)| match a.0.cmp(&b.0) {
                       Ordering::Equal => a.1.cmp(&b.1),
                       x => x,
                   });
    let data = images.into_iter()
                     .fold(Vec::new(),
                           |mut list: Vec<(String, AtlasItem)>, item: (String, i32, Image)| {
                               let len = list.len();
                               // There are no previous items or the previous item is a different
                               // name
                               if len == 0 || list[len - 1].0 != item.0 {
                                   list.push((item.0, AtlasItem::Image(item.2)));
                               } else {
                                   // If the previous entry is a still frame, convert it into a
                                   // 1-sized animation
                                   let is_still = match list[len - 1].1 {
                                       AtlasItem::Image(_) => true,
                                       _ => false,
                                   };
                                   if is_still {
                     ...
                     .into_iter()
                     .collect::<HashMap<String, AtlasItem>>();
    Atlas { data }
}
...
fn parse<P: AsRef<Path>>(data: String, path: P) -> ManifestLoader {
    ...
    fn parse_body(data: String, path: &Path) -> ManifestContents {
        use std::{iter::Map as IterMap, path::PathBuf};
        // Split a line into its right-hand values
        fn get_values_from_line<'a, T: FromStr>(
            line: &'a str)
            -> IterMap<Split<'a, &'static str>, fn(&'a str) -> T>
            where <T as FromStr>::Err: Debug {
            fn parse<'a, T: FromStr>(item: &'a str) -> T
                where <T as FromStr>::Err: Debug {
                item.parse().unwrap()
            }
        ...
        // Get a value from the iterator (repackages Iterator::next into a Result
        ...
        let directory: &Path = if let Some(parent) = path.parent() {
            parent
        } else {
            path.as_ref()
        };
        while let Some(line) = lines.next() {
            ...
            while let Some(line) = lines.next() {
                ...
                let region = Rectangle::new(getval(&mut xy)?,
                                            getval(&mut xy)?,
                                            getval(&mut size)?,
                                            getval(&mut size)?);
                ...
                let center = region.center()
                             + (original_size - region.size() - offset.x_comp() + offset.y_comp());
                let image = images.len() - 1;
                regions.push(Region { image,
                                      name,
                                      region,
                                      rotate,
                                      center,
...
///
/// If there is only one frame / no index for an Atlas item, it will be an
/// Image, otherwise, it will be an Animation.
pub enum AtlasItem {
    /// A still image frame
    Image(Image),
...
impl Display for AtlasError {
    fn fmt(&self, f: &mut Formatter) -> FmtResult { write!(f, "{}", self.description()) }
}
...
impl From<ImageError> for AtlasError {
    fn from(err: ImageError) -> AtlasError { AtlasError::ImageError(err) }
}

#[doc(hidden)]
impl From<ParseIntError> for AtlasError {
    fn from(_: ParseIntError) -> AtlasError { AtlasError::ParseError("Failed to parse an int") }
}

#[doc(hidden)]
impl From<ParseBoolError> for AtlasError {
    fn from(_: ParseBoolError) -> AtlasError { AtlasError::ParseError("Failed to parse an bool") }
}

#[doc(hidden)]
impl From<IOError> for AtlasError {
    fn from(err: IOError) -> AtlasError { AtlasError::IOError(err) }
}
