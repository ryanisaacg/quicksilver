...
impl Backend for GL3Backend {
    ...
    unsafe fn draw(&mut self, vertices: &[Vertex], triangles: &[GpuTriangle]) {
        ...
        if vertex_length > self.vertex_length {
            ...
            gl::BufferData(gl::ARRAY_BUFFER,
                           self.vertex_length as isize,
                           null(),
                           gl::STREAM_DRAW);
            let stride_distance = (VERTEX_SIZE * size_of::<f32>()) as i32;
            // Set up the vertex attributes
            let pos_attrib =
                gl::GetAttribLocation(self.shader, position_string as *const i8) as u32;
            gl::EnableVertexAttribArray(pos_attrib);
            gl::VertexAttribPointer(pos_attrib, 2, gl::FLOAT, gl::FALSE, stride_distance, null());
            let tex_attrib =
                gl::GetAttribLocation(self.shader, tex_coord_string as *const i8) as u32;
            ...
            gl::VertexAttribPointer(col_attrib,
                                    4,
                                    gl::FLOAT,
                                    gl::FALSE,
                                    stride_distance,
            ...
            gl::VertexAttribPointer(use_texture_attrib,
                                    1,
                                    gl::FLOAT,
                                    gl::FALSE,
                                    stride_distance,
        ...
        if self.indices.len() != 0 {
            ...
            if index_length > self.index_length {
                ...
                gl::BufferData(gl::ELEMENT_ARRAY_BUFFER,
                               self.index_length as isize,
                               null(),
                               gl::STREAM_DRAW);
            }
            gl::BufferSubData(gl::ELEMENT_ARRAY_BUFFER,
                              0,
                              index_length as isize,
                              index_data);
    ...
    unsafe fn create_surface(image: &Image) -> SurfaceData
        where Self: Sized {
        let surface = SurfaceData { framebuffer: gl::GenFramebuffer(), };
...
mod gl {
    ...
    pub unsafe fn GetViewport(target: *mut i32) { gl::GetIntegerv(gl::VIEWPORT, target); }

    pub unsafe fn DrawBuffer(buffer: u32) { gl::DrawBuffers(1, &buffer as *const u32); }

    pub unsafe fn DeleteBuffer(buffer: u32) { gl::DeleteBuffers(1, &buffer as *const u32); }

    pub unsafe fn DeleteFramebuffer(id: u32) { gl::DeleteFramebuffers(1, &id as *const u32); }

    pub unsafe fn DeleteTexture(id: u32) { gl::DeleteTextures(1, &id as *const u32); }

    pub unsafe fn DeleteVertexArray(array: u32) { gl::DeleteVertexArrays(1, &array as *const u32); }
