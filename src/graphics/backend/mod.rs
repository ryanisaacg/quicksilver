...
pub(crate) trait Backend {
    unsafe fn new(texture_mode: ImageScaleStrategy) -> Self
        where Self: Sized;
    ...
    unsafe fn bind_surface(surface: &Surface) -> [i32; 4]
        where Self: Sized;
    unsafe fn unbind_surface(surface: &Surface, viewport: &[i32])
        where Self: Sized;
    ...
    unsafe fn clear_color(&mut self, color: Color, letterbox: Color) {
        ...
        self.draw(&[Vertex::new_untextured(Vector::new(-1, -1), color),
...
pub use self::{blend_mode::BlendMode, image_data::ImageData, surface_data::SurfaceData};
...
pub enum ImageScaleStrategy {
    /// The image should attempt to preserve each pixel as accurately as
    /// possible
    Pixelate,
