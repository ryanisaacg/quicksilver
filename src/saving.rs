...
//!
//! On Web, data may only be stored as a cookie in the user's browser. On the
//! desktop, Windows, macOS, and other Unix-style operating systems all have
//! different locations where applications should store data. This module
//! allows any type that implements Serde serialize and deserialize to be saved
//! and loaded.
...
///
///Different platforms may have different save locations: on the Web, data is
/// saved in cookies, on the desktop, it is stored in some appropriate
/// home-directory folder.
///
///The appname should be some constant; this is used to name the file to place
/// the save in on Desktop platforms. The profile should allow multiple saves
/// of the same game (save slots, numbered saves, different players) etc.
...
fn save_impl<T: Serialize>(appname: &str, profile: &str, data: &T) -> Result<(), SaveError> {
    ...
    DirBuilder::new().recursive(true)
                     .create(get_save_folder(appname)?)?;
...
fn save_impl<T: Serialize>(_appname: &str, profile: &str, data: &T) -> Result<(), SaveError> {
    ...
    storage.insert(profile, serde_json::to_string(data)?.as_str())
           .unwrap();
    Ok(())
}
...
pub enum SaveError {
    ...
    IOError(IOError),
    /// The user has no home directory so no save or load location can be
    /// established
    SaveLocationNotFound,
    /// The save profile with the given name was not found
    ///
    /// On desktop this will more likely be reported as an IO error, but on web
    /// it will be a SaveNotFound
    SaveNotFound(String),
}

impl From<SerdeError> for SaveError {
    fn from(err: SerdeError) -> SaveError { SaveError::SerdeError(err) }
}

impl From<IOError> for SaveError {
    fn from(err: IOError) -> SaveError { SaveError::IOError(err) }
}
