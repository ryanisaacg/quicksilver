...
use {
    ...
    stdweb::{unstable::TryInto, web::TypedArray, Value},
};

/// A Future that loads a file into an owned Vec of bytes
///
/// It exists for loading files from the server with Javascript on the web, and
/// providing a unified API between desktop and the web when it comes to file
/// loading
#[derive(Debug)]
...
impl FileLoader {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> FileLoader { FileLoader::new_impl(path) }

    #[cfg(not(target_arch = "wasm32"))]
    fn new_impl<P: AsRef<Path>>(path: P) -> FileLoader {
        FileLoader { path: PathBuf::from(path.as_ref()), }
    }
    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        ...
        match status / 100 {
            ...
            2 => {
                ...
                let response = if let Value::Reference(reference) = response {
                    reference
                } else {
                    unreachable!()
                };
...
impl Error for WasmIOError {
    fn description(&self) -> &str { "An error occurred during a file IO operation" }
}
